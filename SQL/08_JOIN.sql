...
/*
    ...
    # 기본키 (Primary key)
        - 후보키들 중에서 테이블의 키로 선택된것
        - 한 테이블 당 기본키는 하나밖에 설정할 수 없다
    # 외래키 (Foreign key, FK)
        - 다른 테이블에서는 기본키(혹은 후보키)이지만, 
        현재 테이블에서는 중복되는 값인 컬럼
        - employees 테이블의 job_id는 jobs 테이블에서는 기본키이다
        - employees 테이블의 department_id는 departments 테이블에서는 기본키이다
        - 외래키가 설정된 테이블간에는 관계가 형성된다
        
        ※ 다른 테이블의 기본키를 값으로 사용하는 행위는 테이블간의 관계를 형성한다
...
-- 테이블을 적절하게 잘 쪼개놓으면 필요할 때 하나의 테이블로 합쳐서 사용할 수 있고
...
/*
    # OUTER JOIN
        ...
        - 등장하기 못한 행을 확인하고 싶은 쪽의 반대편 조건에 (+)를 붙인다
        - (+)를 붙인쪽의 행에 null을 추가하여 등장하지 못했던 행을 확인한다
*/
SELECT 
    a.last_name as "내이름", b.last_name as "매니저 이름"
FROM
    employees a, employees b
WHERE
    a.manager_id = b.employee_id(+)
...
-- 연습문제07: 사원명/부서번호/부서이름을 출력하되
-- 사원이 한명도 속하지 않은 부서를 조회해보세요
SELECT
