...
/*
    ...
    JOBS : 직급 정보를 저장한 테이블
    LOCATION : 지역 정보를 저자안 테이블
    REGIONS : 지역 정보를 저장한 테이블
*/
...
-- 오라클의 데이터 타입
/*
    # NUMBER(자릿수), NUMBER(전체 자릿수, 소수점 자릿수)
    - 숫자 데이터를 저장할 때 사용하는 컬럼 타입
    - NUMBER(6) : 6자리 정수를 저장할 수 있는 컬럼이 된다
    - NUMBER(6,3) : 6자리 정수와 3자리 소수점을 저장할 수 있는 컬럼이 된다
    # CHAR(자릿수)
...
-- 월급에 보너스 적용해보기
-- 컬럼명이 계산식 때문에 길어지는 겨우 AS를 이용해 컬럼명을 수정할 수 있다
-- null은 존재하지 않는 값이기 때문에 산술연산에 포함되면 무조건 null이 된다
SELECT last_name, salary * commission_pct as 보너스 FROM employees;
SELECT last_name, salary + salary * commission_pct as 총금액 FROM employees;

-- NVL(컬럼명, 대체값) 함수 : 해당 컬럼에서 NULL값이 나왔을때 대체값을 사용한다
SELECT 
...
-- 연습문제02 : 모든 사원들의 사번/이름/연봉을 출력해보세요 (참고로 salary는 월급)
DESC employees;
SELECT
    employee_id, last_name, first_name, salary * 12 as 연봉
FROM employees;
