...
-- 오라클에서 문자열 리터럴을 나타낼때에는 반드시 ''를 사용해야 한다
SELECT * FROM employees WHERE first_name = 'Lisa';
-- 날짜 타입도 문자열 데이터와 크기 비교가 가능하다
...
-- COLUMN BETWEEN A AND B : 해당 컬럼의 값이 A이상 B이하일때 True
SELECT * FROM employees WHERE salary BETWEEN 2000 AND 3000;
SELECT * FROM employees WHERE hire_date BETWEEN '05/07/01' AND '05/12/31';

-- IN (A, B, C ...) : ()안의 내용에 해당 값이 존재하면 true
...
SELECT
    *
FROM employees WHERE phone_number LIKE '%.123.%';

-- LAST_NAME의 두 번째 글자가 a인 모든 행을 조회
SELECT
    *
FROM employees WHERE last_name LIKE '_a%';

-- LAST_NAME에 a가 두 개 이상 포함된 모든 행을 조회
SELECT
    *
FROM employees WHERE last_name LIKE '%a%a%';

-- 연습문제04 : LAST_NAME의 뒤에서 세 번째 글자가 a인 모든 사원을 조회해보세요
SELECT
    *
FROM employees WHERE last_name LIKE '%a__';

-- 연습문제05 : 이름의 3번 째 글자가 r이고 핸드폰 번호에 .이 3개 포함된
--             모든 사원을 조회해보세요
SELECT
    *
FROM employees WHERE first_name LIKE '%r__' 
