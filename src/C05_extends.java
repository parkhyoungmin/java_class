...
public class C05_extends {
	...
	public static void main(String[] args) {
		...
		minsu.sayHi(); // 안녕! 내 이름은 민수이고, 나이는 10살이야,
		
		new Police().sayHi();
		System.out.println(new Police().name);;
		
		Police police01 = new Police();
		Doctor doctor01 = new Doctor(); // 아무것도 입력하지 않았지만 상속으로 인해 sayHi는 출력된다 
		...
		// 아무 문제없이 Person타입이 될 수 있다 (업캐스팅)
		// ※ 부모타입으로 올라가는 업캐스팅은 아무 문제가 없다
		
		// Person은 자식인 Police에 모르는 기능이 존재할 수 있기 때문에
		// 자식 타입이 될수없다 (다운캐스팅)
		// ※ 다운 캐스팅은 특별한 경우를 제외하고는 불가능하다 (자식 -> 업캐스팅 -> 부모 -> 다운캐스팅 -> 자식)
		Person person01 = new Police();
		Person person02 = new Dentist(); // Person person02 -> Dentist로 업캐스팅 
...
//		Police police2 = (Police) person02; // dentist(person2)를 police로 바꾸려고 해서 에러가 났음
...
//		  부모의 생성자를 가장 먼저 호출해야 한다
	...
	public Police() {
		super("김경찰", 25); // 부모의 생성자가 값을 요구하기때문에
		// this() : 현재 인스턴스의 생성자
...
// 기본 생성자 잠금 효과
// 다른 타입으로 업캐스팅
// damage()를 사용할때 업캐스팅되었던 자식 인스턴스의 damage를 사용했다
