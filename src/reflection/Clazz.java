...
public class Clazz {
	...
	SecurityException, ClassNotFoundException, NoMoreCalorieException {
		...
		System.out.println("forName으로 불러온 Class이름 : " + c2.getName());
		...
		try {
			...
			System.out.println("c3는 인터페이스인가요? " + c3.isInterface());
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		...
		((myobj.fruit.Apple)apple02).eat().eat();
		
		Object apple03 = c.getConstructor(
				Class.forName(""),
				Class.forName("java.lang.String")
		).newInstance(900, "Green");
		((myobj.fruit.Apple)apple03).eat().eat();
